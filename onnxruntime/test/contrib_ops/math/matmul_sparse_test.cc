...
void ConvertToCsr(gsl::span<const T> input_span,
  ...
  ASSERT_EQ(input_span.size(), dense_size);
...
TEST(SparseToDenseMatMul, TestCsr) {
  ...
  constexpr int64_t dense_size = rows * cols;
  ...
  ConvertToCsr<float>(gsl::make_span(input_data), A_shape, A_values, A_inner_indices, A_outer_indices);
